...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchPosts = async() => {
            try {
                const posts = await request(`/api/v1/find/userposts/${id}`, 'GET')
                console.log(posts)
                setUserPosts(posts)
            } catch (error) {
                console.error(error)
    ...
    useEffect(() => {
        const fetchUserData = async() => {
            const user = await request(`/api/v1/find/${id}`, 'GET')
    ...
    const handleFollow = async() => {
        try {
            ...
            if(isFollowed){
                await request(`/api/v1/unfollow/${id}`, "PUT", headers)
            } else {
                await request(`/api/v1/follow/${id}`, "PUT", headers)
            }
