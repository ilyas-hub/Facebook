...
const Post = ({ post }) => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const data = await request('/api/v1/find/' + post.userId, 'GET')
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const data = await request(`/api/v1/${post._id}`, 'GET')
        console.log(data)
        setComments(data)
      } catch (error) {
        console.error(error)
  ...
  const handleLike = async () => {
    ...
    try {
      await request(`/api/v1/likePost/${post._id}`, "PUT", headers)
  ...
  const handleDislike = async () => {
    ...
    try {
      await request(`/api/v1/dislikePost/${post?._id}`, "PUT", headers)
      setIsLiked(prev => !prev)
    } catch (error) {
      console.error(error)
    }
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      const data = await request('/api/v1', 'POST', headers, { text: commentText, postId: post._id })
      console.log(data)
      setComments(prev => {
  ...
  const handleDeletePost = async() => {
    try {
      ...
      await request('/api/v1/deletePost/' + post._id, 'DELETE', headers)
